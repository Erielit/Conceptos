...
public class MetodosBusqueda {
    ...
    public static void main(String[] args) {
        ...
        busquedaSecuencial(arr, 10);
    }

    public static void busquedaSecuencial(int[] arr, int num) {
        boolean error = true;
        for (int i = 0; i < arr.length; i++) {//recorremos todo el arreglo
            if (arr[i] == num) {//comparamos el elemento en el arreglo con el buscado
                System.out.println("Número enontrado: " + num + ", con posición: " + i);//Si es verdadero guardamos la posicion
                error = false;
                break;
            }
        }
    ...
    public static void busquedaBinaria(int[] arr, int num) {
        ...
        while (first <= last) {
            centro = (first + last) / 2;
            if (arr[centro] == num) {
                System.out.println("Número enontrado: " + num + ", con posición: " + centro);
                error = false;
                break;
